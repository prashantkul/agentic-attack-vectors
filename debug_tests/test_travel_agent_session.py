...
async def test_travel_agent_session_context():
    ...
    for i, message in enumerate(conversations, 1):
        ...
        if i >= 4:
            context_indicators = ["sarah", "Sarah", "$3000", "3000", "budget", "outdoor", "nature"]
    ...
    interests_remembered = "outdoor" in response4.lower() or "nature" in response4.lower()
    
    contextual_recommendation = any(word in response5.lower() for word in ["sarah", "budget", "outdoor", "nature", "hiking", "park"])
...
async def main():
    ...
    try:
        ...
        if result:
            ...
            print("   ✅ Ready for memory poisoning tests with proper context")
        else:
            print("\n💥 FAILED: Travel Agent not maintaining session context")
            print("   ❌ Need to fix agent configuration before memory poisoning tests")
