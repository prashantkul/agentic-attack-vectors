...
class CustomMemoryService:
    ...
    def _init_database(self):
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_conv_user_app ON conversations(user_id, app_name)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_memory_user_app ON memory_summaries(user_id, app_name)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_memory_relevance ON memory_summaries(relevance_score DESC)")
        
        conn.commit()
        conn.close()
        
    async def store_conversation(self, user_id: str, app_name: str, session_id: str, 
                                user_message: str, agent_response: str, metadata: Dict = None):
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (user_id, app_name, session_id, 'user', user_message, json.dumps(metadata or {})))
            
            # Store agent response
            cursor.execute("""
                INSERT INTO conversations (user_id, app_name, session_id, message_type, content, metadata)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (user_id, app_name, session_id, 'agent', agent_response, json.dumps(metadata or {})))
            
            conn.commit()
            
            # Generate memory summaries asynchronously
            await self._generate_memory_summaries(user_id, app_name, user_message, agent_response)
            
        finally:
            conn.close()
            
    async def _generate_memory_summaries(self, user_id: str, app_name: str, 
                                       user_message: str, agent_response: str):
        """Generate memory summaries from conversation for efficient retrieval."""
        
        # Extract preferences
        preference_keywords = ['prefer', 'like', 'love', 'want', 'need', 'budget', 'favorite']
        if any(keyword in user_message.lower() for keyword in preference_keywords):
            await self._store_memory_summary(user_id, app_name, 'preference', user_message)
        ...
        await self._store_memory_summary(user_id, app_name, 'context', context_summary, relevance_score=0.7)
        
    async def _store_memory_summary(self, user_id: str, app_name: str, memory_type: str, 
                                  summary: str, relevance_score: float = 1.0):
    ...
    async def retrieve_memories(self, user_id: str, app_name: str, query: str = None, 
    ...
    async def inject_malicious_memory(self, user_id: str, app_name: str, 
                                    malicious_content: str, memory_type: str = 'preference',
        ...
        await self._store_memory_summary(user_id, app_name, memory_type, 
                                       malicious_content, relevance_score)
        ...
        await self.store_conversation(
            user_id, app_name, fake_session_id,
        ...
        logger.warning(f"Injected malicious memory for user {user_id}: {malicious_content[:50]}...")
        
    async def contaminate_cross_user(self, source_user_id: str, target_user_id: str, 
                                   app_name: str, contamination_data: str):
        """
        ...
        await self.inject_malicious_memory(
            target_user_id, app_name, contamination_data, 
            memory_type='contamination', relevance_score=0.9
        )
        
        logger.warning(f"Cross-user contamination: {source_user_id} -> {target_user_id}")
    ...
    async def clear_user_memory(self, user_id: str, app_name: str = None):
        ...
        try:
            if app_name:
                cursor.execute("DELETE FROM conversations WHERE user_id = ? AND app_name = ?", 
                             (user_id, app_name))
                cursor.execute("DELETE FROM memory_summaries WHERE user_id = ? AND app_name = ?", 
                             (user_id, app_name))
            else:
...
class GroqMemoryAgent:
    ...
    async def process_with_memory(self, user_id: str, app_name: str, session_id: str, 
                                user_message: str) -> str:
        """Process user message with memory context."""
        
        # Retrieve relevant memories
        memories = await self.memory_service.retrieve_memories(user_id, app_name, user_message)
...
async def create_groq_memory_agent(travel_agent, db_path: str = None) -> GroqMemoryAgent:
